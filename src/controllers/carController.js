...
const CarImportService = require('../services/carImportService');
const { isValidImageSignature, uploadDir } = require('../utils/imageValidation');

const appRoot = path.resolve(__dirname, '..', '..');

// Ruta relativa importada válida: /uploads/<32 hex><extensión permitida>
// (formato exacto que genera tanto Multer como el importador por URL).
const IMPORTED_IMAGE_PATH_RE = /^\/uploads\/[a-f0-9]{32}\.(jpg|jpeg|png|webp)$/i;
...
const CarController = {
    ...
    // Reglas de validación para la importación de una ficha desde una URL externa.
    validateImportUrl: [
        body('url').trim().notEmpty().withMessage('Indica la URL del anuncio.')
            .isURL({ protocols: ['https'], require_protocol: true }).withMessage('La URL debe ser una dirección HTTPS válida.')
            .isLength({ max: 2000 }).withMessage('La URL es demasiado larga.')
    ],
    ...
    // Importa los datos de una ficha de vehículo (marca, modelo, año, precio,
    // fotos...) desde una URL de un portal soportado, para autorellenar el
    // formulario de alta. No guarda nada en el catálogo: el admin siempre
    // revisa/edita los campos y confirma con "Registrar en Catálogo".
    async importFromUrl(req, res) {
        ...
        try {
            const data = await CarImportService.importFromUrl(req.body.url);
            logger.info(`Administrador '${req.session.adminUser}' importó ficha desde URL: ${data.fuenteUrl}`);
            res.json({ ok: true, data });
        } catch (err) {
            if (err instanceof CarImportService.ImportError) {
                return res.status(422).json({ ok: false, error: err.message });
            }
            logger.error('Error al importar coche desde URL: ' + err.message, { error: err });
            res.status(500).json({ ok: false, error: 'No se pudo importar la ficha del vehículo. Rellena el formulario manualmente.' });
        }
    },
    ...
    async createCar(req, res) {
        ...
        try {
            ...
            // Imágenes ya descargadas y validadas por el importador desde URL
            // (ver CarController.importFromUrl): solo se aceptan rutas con el
            // formato exacto generado por el propio servidor y que existan
            // realmente en el directorio de subidas, para no confiar a ciegas
            // en un campo oculto que llega desde el navegador.
            let importedImages = [];
            if (req.body.imagenes_importadas) {
                try {
                    const parsed = JSON.parse(req.body.imagenes_importadas);
                    if (Array.isArray(parsed)) {
                        importedImages = parsed.filter(p => {
                            if (typeof p !== 'string' || !IMPORTED_IMAGE_PATH_RE.test(p)) return false;
                            return fs.existsSync(path.join(uploadDir, path.basename(p)));
                        });
                    }
                } catch (err) {
                    // Campo corrupto/manipulado: se ignora, no aborta el alta
                }
            }

            const uploadedFileUrls = (req.files || []).map(f => `/uploads/${f.filename}`);
            const imageUrls = [...uploadedFileUrls, ...importedImages];

            if (imageUrls.length === 0) {
                cleanupUploadedFiles();
                return res.status(400).render('error', {
                    message: 'Debes subir o importar al menos una imagen del vehículo.',
                    title: 'Error de Validación'
                });
            }
